...
numberAdvance=int(temp[1])

allCows=[]
for i in range(numberCows):
  temp=input().split()
  allCows.append([int(temp[0]),int(temp[1]),i+1])

allCows.sort()
#print(allCows)
newCows=[]
for i in range(numberCows-1,numberCows-1-numberAdvance,-1):
  currArr=allCows[i]
  newCows.append([currArr[1],currArr[0],currArr[2]])

newCows.sort()
#print(allCows)

print(newCows[numberAdvance-1][2])
